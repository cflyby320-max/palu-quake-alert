...
- **Evergreen educational track (`--edu`):** for the quiet periods between quakes (the activation problem), studio also drafts **preparedness/literacy** cards & carousels from a pillar-structured content bank (`studio/content/bank.js`; calendar in `studio/content/CALENDAR.md`; built from the Sulawesi Tengah content strategy — 3 pillars + the 28-Sep anniversary). `node studio/studio.js --edu` lists posts; `--edu <id> [--dry-run] [--llm]` renders one (card or carousel) and DMs the draft for **manual** posting to the channel + Instagram; `--edu-all` does the batch. Educational cards share the exact brand chrome + honest-framing footer with the quake card via `studio/svg.js`. Captions are **pre-authored** (no LLM by default; qualified 2018 figures and BMKG/BNPB/BPBD routing baked in); the opt-in `--llm` flag rephrases via the Claude API but is gated by `validateEduCaption` with a deterministic fallback. Offline safety tests: `test/studio_content.test.js`.
